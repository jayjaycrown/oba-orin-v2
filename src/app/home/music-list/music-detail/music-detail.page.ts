...
export class MusicDetailPage implements OnInit {
  ...
  next() {
    ...
    this.route.paramMap.subscribe((params) => {
      if (!params.has('id')) {
        this.navCtrl.navigateBack('/home/music-list');
      }
      this.id = params.get('id');
      let next = parseInt(this.id) + 1;
      // alert(next);
      this.router.navigateByUrl('home/music-detail/' + next);
  ...
  previous() {
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      // alert(previous);
      this.router.navigateByUrl('home/music-detail/' + previous);
      // alert(window.location.pathname);
    });
  }
